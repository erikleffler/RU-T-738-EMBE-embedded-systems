...
void Operational_state::on_do()
{
    ...
    {
        if(this->context_->is_reader)
        {
            // is reader
            input_ph = this->context_->photocell_ptr->read(); 
            this->context_->filter_ptr->photocell_val = this->context_->filter_ptr->smooth(input_ph);
            this->context_->led_ptr->set_lo();   // low
        }
        else
        {
            // is not reader
            this->context_->led_ptr->set_val(this->context_->filter_ptr->photocell_val);
        }
    }
